...
import com.secondbot.notification_bot.service.contract.*;
...
public class NotificationManager extends AbstractManager
    ...
    public BotApiMethod<?> mainMenu(CallbackQuery query, Bot bot) {
        return EditMessageText.builder()
                ...
                .text("тут пака ничего. иди дальше")
                .replyMarkup(
    ...
    private BotApiMethod<?> askSeconds(CallbackQuery query, String id) {
        ...
        userRepo.save(user);
        return EditMessageText.builder()
                .text("⚡\uFE0F Введи время, через сколько хочешь получить напоминание\nФормат - ЧЧ:ММ:СС\nНапример - (01:30:00) - полтора часа")
                .messageId(query.getMessage().getMessageId())
                .chatId(query.getMessage().getChatId())
                .replyMarkup(
                        keyboardFactory.createInlineKeyboard(
    ...
    private BotApiMethod<?> askDescription(CallbackQuery query, String id) {
        ...
        userRepo.save(user);
        return EditMessageText.builder()
                .text("⚡\uFE0F Добавь или измени описание, просто напишите в чат тест, который бы хотел получить")
                .messageId(query.getMessage().getMessageId())
                .chatId(query.getMessage().getChatId())
                .replyMarkup(
                        keyboardFactory.createInlineKeyboard(
    ...
    private BotApiMethod<?> askTitle(CallbackQuery query, String id) {
        ...
        userRepo.save(user);
        return EditMessageText.builder()
                .text("⚡\uFE0F Опиши заголовок в следующем сообщении")
                .messageId(query.getMessage().getMessageId())
                .chatId(query.getMessage().getChatId())
                .replyMarkup(
                        keyboardFactory.createInlineKeyboard(
